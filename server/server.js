...
databaseQuery = (req, res) => {
	...
	var query2 = '(SELECT code_ID, count(*) AS totalNum, count(dx_criteria) AS selectedNum, Round((count(dx_criteria)/count(*))*100, 2) AS accuracy FROM  '
	var query3 = '(SELECT DISTINCT symptom, dx_criteria, code_ID, IF(dx_criteria IS NULL, "N/A", "MET") AS `dx_criteria_result` FROM '
	var query4 = '(SELECT DISTINCT symptom, code_ID FROM MentalHealth.symptom_description  '
	var query5 = 'WHERE code_ID IN '
	var query6 = '(SELECT code_ID FROM MentalHealth.symptom_description WHERE symptom_ID IN '
	var query7 = '(SELECT DISTINCT SE_ID FROM MentalHealth.questions_tbl '
	var query8 = 'LEFT JOIN symptom_tbl ON questions_tbl.SE_ID = symptom_tbl.symptom_ID '
	var query9 = 'WHERE SymptomOrEvidence = "S" AND question_ID IN ('

	var query10 = ')))) AS `A` LEFT JOIN '
	var query11 = '(SELECT dx_criteria FROM MentalHealth.questions_tbl '
	var query12 = 'LEFT JOIN symptom_tbl ON questions_tbl.SE_ID = symptom_tbl.symptom_ID '
	var query13 = 'WHERE SymptomOrEvidence = "S" AND question_ID IN ('

	var query14 = ')) AS `B`ON A.symptom = B.dx_criteria) AS `C` '
